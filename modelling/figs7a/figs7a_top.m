...
for i = 1:50
        res(i).delta_lif = i * 0.1;
        res(i).delta_hh = i * 0.5;
        delta_lif(i) = i * 0.1;
        delta_hh(i) = i * 0.5;
...
for i = 1:50
    if i <= 50
        ...
        parfor t = 1:tottrail
            if t<=tottrail
                [~,~,st_lif_sc] = LIF_SC('layer',10,'delta',delta_lif(i));
                [~,~,~,st_lif_fd] = LIF_FD('layer',4,'delta',delta_lif(i));
                [~,~,st_hh_sc] = HH_SC('layer',10,'delta',delta_hh(i));
                [~,~,~,st_hh_fd] = HH_FD('layer',4,'delta',delta_hh(i));
