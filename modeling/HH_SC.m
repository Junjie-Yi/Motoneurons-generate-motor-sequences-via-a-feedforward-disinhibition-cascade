...
%% synaptic current
dsdt = @(v,s) ((1-s)./(1+exp(-(v+20)/4)) - s)/tau;
kt = 0:dt:40;
...
for i = 1:1
    ...
    inputlayer(i).h = ones(neuron,step)*0.22;
...
for i = 1:totlayer
    ...
    exclayer(i).h = ones(neuron,step)*0.22;
...
for i = 1:totlayer
    ...
    motorlayer(i).h = ones(1,step)*0.22;
    motorlayer(i).Iext = zeros(1,step);
...
for l = 1:totlayer
    ...
    for i = 2:step
        ...
        m = exclayer(l).m(:,i-1);
        h = exclayer(l).h(:,i-1);
        Iext = exclayer(l).Iext(:,i-1);
        Ibg = exclayer(l).Ibg(:,i-1);
        
        if l>1
            Isyn = exce*exclayer(l-1).output(i-1).*(v-Ve)/neuron;
        else
